...
def rnn_step_backward(dnext_h, cache):
    ...
    dz =  dnext_h * 4/((np.exp(z) + np.exp(-z)) ** 2)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_prev = h0 
    for t in range(T):
      h_next, cache[t] = rnn_step_forward(x[:,t,:], h_prev, Wx, Wh, b)
      h[:,t,:] = h_next 
      h_prev = h_next 
...
def rnn_backward(dh, cache):
    ...
    for t in range(T,0,-1):
      dht += dh[:,t-1,:]
      dxt,dht,dWxt,dWht,dbt = rnn_step_backward(dht,cache[t-1])
      dx[:,t-1,:] = dxt 
      dWx += dWxt 
      dWh += dWht
      db += dbt
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (A, i, f, o, g, prev_c,prev_h,next_c,next_h, x, Wx, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    A, i, f, o, g, prev_c, prev_h, next_c, next_h, x, Wx, Wh = cache 
    _,H = prev_c.shape 
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((N,T,H))
    for t in range(T):
      next_h, next_c, cache[t] = lstm_step_forward(
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
      dnext_h += dh[:,t,:]
      dxt,dprev_h,dprev_c,dWxt,dWht,dbt = lstm_step_backward(dnext_h,dnext_c,cache[t]) 
      dx[:,t,:] = dxt 
      dnext_c = dprev_c
      dnext_h = dprev_h
      dWx += dWxt 
