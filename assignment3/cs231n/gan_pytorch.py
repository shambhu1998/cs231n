...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_real, torch.ones(logits_real.size()).type(dtype)) \
           + bce_loss(logits_fake, torch.zeros(logits_fake.size()).type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * torch.mean((scores_real - 1).pow(2)) \
           + 0.5 * torch.mean((scores_fake).pow(2))
