...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        for i in range(max_len):
          for j in range(0, embed_dim, 2):
            ...
            pe[:,i,j+1] = math.cos(i / (10000 ** ((j)/embed_dim)))
    ...
    def forward(self, x):
        ...
        output = x + torch.autograd.Variable(self.pe[:,:S,:], requires_grad=False)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.y_dropout = nn.Dropout(dropout)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        K = self.key(key).view(N, -1, H, self.d_k)
        Q = self.query(query).view(N, -1, H, self.d_k)
        V = self.value(value).view(N, -1, H, self.d_k)
        
        K = K.transpose(1,2)
        Q = Q.transpose(1,2)
        V = V.transpose(1,2)
        
        # calculating attension
        ...
        if self.dropout is not None:
          scores = self.y_dropout(scores)
        # print(scores.shape)
        output = torch.matmul(scores, V)
        # print(output.shape)
        concat = output.transpose(1,2).contiguous().view(N, -1, self.embed_dim)
