...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
          self.params[f'W{i+1}'] = np.random.randn(all_layers[i], all_layers[i+1]) * weight_scale
          self.params[f'b{i+1}'] = np.zeros(all_layers[i+1])
    ...
    def loss(self, X, y=None):
        ...
        # Final layer affine backwards
        dout, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = \
          affine_backward(ds, affine_cache.pop())
        ...
        while i >= 0:
          # Dropout backwards
          if self.use_dropout: 
            dout = dropout_backward(dout, dropout_cache.pop())
          # Relu backwards
          dout = relu_backward(dout, relu_cache.pop())
          # Batchnorm backwards
          if self.normalization == 'batchnorm':
            dout, grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = batchnorm_backward(dout, bn_cache.pop())
          # Layernorm backwards 
          if self.normalization == 'layernorm':
            dout, grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = layernorm_backward(dout, ln_cache.pop())
          # Affine backwards
          dout, grads[f'W{i+1}'], grads[f'b{i+1}'] = affine_backward(dout, affine_cache.pop())
          # Adding regularization term to loss and grads
