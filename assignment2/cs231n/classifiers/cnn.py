...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        C,H,W = input_dim
        #hieght and width after pooling layer
        HP = 1 + (H-2)//2
        WP = 1 + (W-2)//2
    ...
    def loss(self, X, y=None):
        ...
        # convolution-relu-pooling cache = (conv_cache, relu_cache, pool_cache)
        out, cache1 = conv_relu_pool_forward(X,W1,b1,conv_param, pool_param)
        # flatten the output to feed into affine layer
        shape_before_flat = out.shape 
        out = out.reshape(out.shape[0],-1)
        # passing to affine relu
        ...
        #first affine backward
        da2, grads['W3'],grads['b3'] = affine_backward(ds, cache3)
        grads['W3'] += self.reg * W3
        # affine-relu backward
        da1, grads['W2'],grads['b2'] = affine_relu_backward(da2, cache2)
        grads['W2'] += self.reg * W2
        # con-relu-pool backward
        da1 = da1.reshape(shape_before_flat)
